...
def generic_url_maker(instance, action, id, root_object=False):
    ...
    if not root_object:
        ...
        url = six.text_type(APP_ROOT_URL)              + \
            six.text_type(instance._meta.app_label)  + "/" + \
            six.text_type(instance.__model_label__)  + "/" + \
            six.text_type(action) + "/" + six.text_type(id) + "/"
    if root_object:
        url = six.text_type(APP_ROOT_URL) + six.text_type(
            instance._meta.app_label) + "/" + six.text_type(action) + "/"
    return url
